...
int split(Informacoes dados[], int beg, int ed)
{
    ...
    //função para separar dividir um vetor em dois novos vetores, um com os elementos menores que o pivo e outro com os maiores
    Informacoes pivot = dados[ed]; //estratégia basica para escolher o pivo, usando o ultimo indice
    int i = beg;
...
void quickSort(Informacoes dados[], int beg,int ed)    // método nao estavel o resultado sera o vetor dados[] modificado
{
    if(beg<ed)
    {
        comparacoes++;
...
void preprocess(Informacoes dados[],int n){
    ...
    for(int i=0;i<n;i++){
        ...
        if(cidade_atual==dados[i].getCidade()){ //ja tem registros dessa cidade, entao o calculo dos casos diarios é feito
               if(dados[i].getCasos()>casosAteData){
                ...
                cidade_atual=dados[i].getCidade();      //asumindo que o primeiro registro da cidade corresponde com o primeiro dia que teve infecções
...
void mergeEstado(Informacoes dados[], int l, int m, int r)
{
    ...
    // copiando dados p estruturas temporárias
    for (int i = 0; i < n1; i++)
        L[i] = dados[l + i];
    for (int j = 0; j < n2; j++)
        R[j] = dados[m + 1 + j];
    // Mesclar as variaveis temporárias de volta em dados[]
    int i = 0;
    int j = 0;
...
void leArquivoPreProcessado(Informacoes dados[],int n,int nMax, int opcao)
{
    ...
    if(opcao==1){
        ...
        ofstream arq("saida.txt");
         arq << "IMPORTAÇÃO DE 100 REGISTROS ALEATÓRIOS"<< endl;
            for(int i=0; i<n; i++)
    {
...
void leArquivoRaw(Informacoes dados[],int n)
{
